...
This script trains a LightGBM model for multi-horizon Bitcoin price forecasting
...
def main():
    """Main LightGBM training process"""
    print("=" * 80)
    print("LIGHTGBM MODEL TRAINING - BITCOIN FORECASTING")
    print("=" * 80)
    print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    logger = setup_logging('lightgbm_training.log')
    ...
    try:
        ...
        try:
            ...
            print("Please install with: pip install lightgbm")
            sys.exit(1)
        ...
        metrics_file = os.path.join(
            ...
            OUTPUT_CONFIG['metrics_file_template'].format(model_name='lightgbm')
        )
        ...
        final_forecaster = LightGBMForecaster()
        ...
        visualizer.plot_predictions_vs_actual(
            ...
            save_path=os.path.join(plots_dir, 'lightgbm_predictions.png')
        )
        
        visualizer.plot_error_metrics(
            final_metrics,
            save_path=os.path.join(plots_dir, 'lightgbm_error_metrics.png')
        )
        ...
        logger.info("LightGBM training interrupted by user")
        ...
        print("Check the log file 'lightgbm_training.log' for detailed error information.")
        sys.exit(1)
